...
gl.vertexAttrib4f(colAttrib, 0.58,0.32,0.97,1);
gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, iboTris);
gl.drawElements(gl.TRIANGLES,
    iboTris.numberOfElements, gl.UNSIGNED_SHORT, 0);

// Setup rendering lines.
gl.vertexAttrib4f(colAttrib, 0.07, 0.05, 0.1, 1);
gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, iboLines);
...
function createVertexData(){
    ...
    for(var i=0, u=0; i <= n; i++, u += du) {
          ...
          for(var j=0, v=0; j <= m; j++, v += dv) {
            ...
            var x = (Math.cos(u) * (2 * v / Math.PI - Math.tanh(v))) * c;
            var y = (Math.cos(u + 2 * Math.PI / 3) / Math.cosh(v)) * c;
            var z = (Math.cos(u - 2 * Math.PI / 3) / Math.cosh(v)) * c;
